...
// UNIFORM
////////////////////////////////////////////////////////////////////////////////
uniform mat4 gvModelMatrix, objectModelMatrix, cubeModelMatrix;
...
uniform vec3 worldLight;//world coordinates
uniform vec3 lightPos;//camera coordinates
...
// To access info about the cube map
...
uniform bool hasTex;
uniform sampler2D samplerd;//for potential texturing of the object
    ...
    while ( tTree < tMax && /*stop criteria*/accColor.a < 0.99 && numLoop < 200 )
    {
        ...
        if ( brickAddress != 0 )
        {
		    ...
		    while ( tTree < tEnd && /*stop criteria*/accColor.a < 0.99 )
            {
                ...
                vec3 lightVec = normalize( vec3( worldLight ) - samplePosTree );
...
//
// Computes Phong lighting and shadows cast by a GigaVoxels object
////////////////////////////////////////////////////////////////////////////////
void main()
{	
	/***Phong Lighting***/
	...
	vec4 lcolor;
	if (hasTex) {
		lcolor = texture(samplerd, gl_TexCoord[0].xy);
	} else {
		lcolor = vec4(0.5, 0.5, 0.5, 1.0);//color of the object
	}
	vec3 pointToLight = vec3(lightPos - cameraPixelPos);
	...
	diffuse = max(0.0, dot (N, L))*lightIntensityDiff*lcolor;

	vec3 reflVec = reflect(-L, N);
	vec3 EV = normalize(-cameraPixelPos);
	...
	vec4 backdiffuse = max(0.0, dot (-N, L))*lightIntensityDiff*lcolor;
	...
	vec4 backlitColor = ambient + backdiffuse + backspecular;
	
	if (gl_FrontFacing) {
		gl_FragColor = litColor;
	} else {
		gl_FragColor = backlitColor;
	}


	/***Reflection***/
	//Sampling the cube map
	vec4 rayTracedColor;
	vec3 worldNormal = normalize(vec3(inverse(gvModelMatrix)*vec4(objectNormal, 0.0)));
	vec3 cameraToPixel = normalize(vec3(inverse(gvModelMatrix)*(worldPixelPos)) - vec3(inverse(gvModelMatrix)*vec4(worldCamPos, 1.0)));
	vec3 worldRayDir = reflect(cameraToPixel, worldNormal);
	vec4 reflColor = mix(texture(s, vec3(inverse(cubeModelMatrix)*vec4(worldRayDir, 0.0))), litColor, 0.8);
	gl_FragColor = reflColor;

	//Intersection with the Gigavoxels box
	vec3 boxMin = vec3(0.001, 0.001, 0.001);
	vec3 boxMax = vec3(0.999, 0.999, 0.999 );
	float tnear, tfar;
	bool hit = intersectBox( vec3(inverse(gvModelMatrix)*(worldPixelPos)), worldRayDir, boxMin, boxMax, tnear, tfar );
	...
	if ( hit ) {
		...
		float initT = tnear + 1.0 / 512.0;
		float coneFactor = max( 1.0 / 2048.0, lodLevelCone );
		//object traversal: collecting the accumulated color
		rayTracedColor = traceVoxelConeRayCast( vec3(inverse(gvModelMatrix)*(worldPixelPos)), worldRayDir, initT, coneFactor, tfar );
		if (rayTracedColor.w > 0.0) {
			gl_FragColor = mix(rayTracedColor, reflColor, 0.8);
		}
	}
}
