...
{	
	...
	if ( hit ) {
		...
		if (dot(worldRayDir, gvToLight) > 0) {
			rayTracedColor = traceVoxelConeRayCast( vec3(inverse(gvModelMatrix)*(worldPixelPos)), worldRayDir, initT, coneFactor, tfar );
			vec4 shadowColor = vec4(min(rayTracedColor.x*(1-rayTracedColor.w), 0.2),
									min(rayTracedColor.y*(1-rayTracedColor.w), 0.2),
									min(rayTracedColor.z*(1-rayTracedColor.w), 0.2),
									rayTracedColor.w);
