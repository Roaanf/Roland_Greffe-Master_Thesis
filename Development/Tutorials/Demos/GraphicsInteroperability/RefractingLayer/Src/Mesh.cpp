	...
	for (int i = 0; i < scene->mNumMeshes; i++) {
		...
		for (int j = 0; j < mesh->mNumVertices; j++) {
			...
			if (mesh->HasPositions()) {
				...
				M.Vertices.push_back(pos->x - innerShellDistance*normal->x);
				if (boxMax[0] < pos->x) {
					boxMax[0] = pos->x;
				}
				if (boxMin[0] > pos->x) {
				...
				M.Vertices.push_back(pos->y - innerShellDistance*normal->y);
				if (boxMax[1] < pos->y) {
					boxMax[1] = pos->y;
				}
				if (boxMin[1] > pos->y) {
				...
				M.Vertices.push_back(pos->z - innerShellDistance*normal->z);
				if (boxMax[2] < pos->z) {
					boxMax[2] = pos->z;
				}
				if (boxMin[2] > pos->z) {
...
void Mesh::renderMesh(int i) {
	...
	/*if (meshes[i].hasSTextures) {
		//cout << "hasSTextures mesh num"<<i<<endl;gl_texture0
...
void Mesh::setInnerShellDistance(float v) {
	innerShellDistance = v;
}

void Mesh::setProgram(GLuint p) {
	program = p;
}
