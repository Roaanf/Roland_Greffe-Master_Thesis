...
uniform sampler2DRect depthMinTex;
uniform sampler2DRect depthMaxTex;
uniform sampler2DRect normalTex;
uniform sampler2DRect OdepthMinTex;
uniform sampler2DRect OdepthMaxTex;
uniform sampler2DRect OnormalTex;
...
vec4 traceVoxelConeRayCast( vec3 pRayStart, vec3 pRayDir, float t, float pConeFactor, float tMax , vec3 innerNormal);
...
// @param pConeFactor ...
// @param tMax ...
// @param innerNormal normal to consider for lighting the inner object
//
// @return ...
////////////////////////////////////////////////////////////////////////////////
							...
							vec3 innerNormal)
{
    ...
    while ( tTree < tMax && /*stop criteria*/accColor.a < 0.99 && numLoop < 200 )
    {
	        ...
	        //if ( (brickAddress != 0 && !refraction) || ( brickAddress != 0 && condition) || otherCondition)
	        	if ( brickAddress != 0) 
	        {
				// Ray step
			    ...
			    while ( tTree < tEnd && /*stop criteria*/accColor.a < 0.99 )
	            {
					...
					vec4 projPos = P*MV*vec4(samplePosTree, 1.0)/(P*MV*vec4(samplePosTree, 1.0)).w;
					//screen coordinates of current position
					vec2 sampleScreenCoords = vec2( viewport.z*0.5*projPos.x + viewport.x + viewport.z*0.5,
												viewport.w*0.5*projPos.y + viewport.y + viewport.w*0.5 );

					float startInner = texture(depthMinTex, sampleScreenCoords);
					float endInner = texture(depthMaxTex, sampleScreenCoords);

					/***** COLOR ONLY ACCUMULATED WITH CERTAIN CONDITIONS*****/
					//if current position is inside the inner object
					bool condition = startInner != 0.0 && length(samplePosTree - uViewPos) >= startInner && length(samplePosTree - uViewPos) <= endInner;

					if (condition  ) {
						//Lighting (diffuse)
			        	vec3 lightVec = normalize( lightPos - samplePosTree );	  
						color.rgb = color.rgb * max( 0., dot( texture(normalTex, sampleScreenCoords).xyz, lightVec ) );  
						// Accumulate color
...
void main()
{
	//viewing direction (eye to pixel)
    vec3 viewDir = normalize( uViewPlane + uViewAxisX * gl_FragCoord.x + uViewAxisY * gl_FragCoord.y );

    //Outer object stuff
    float Ostart = texture(OdepthMinTex, gl_FragCoord.xy);//distance to camera of the front part of the outer object
    vec3 Onormal = normalize(vec3(texture(OnormalTex, gl_FragCoord.xy)));//normal at current pixel of outer object
    
    //Inner object stuff
    float start = texture(depthMinTex, gl_FragCoord.xy);//distance to camera of the front part of the inner object
   	float finish = texture(depthMaxTex, gl_FragCoord.xy);//distance to camera of the back part of the inner object
   	vec3 normal = normalize(vec3(texture(normalTex, gl_FragCoord.xy)));// normal at current pixel of inner object

   	//'Rendering' cube map
  	mat3 rotCube;//matrix to take the cube map's rotation into account
  	rotCube[0] = vec3(-1.0, 0.0, 0.0);
  	rotCube[1] = vec3(0.0, -1.0, 0.0);
 	rotCube[2] = vec3(0.0, 0.0, 1.0);
  	gl_FragColor = texture(cubeTex, rotCube*viewDir);

	// Refraction
    vec3 pos;
    vec4 refrColor; 	
	if (Ostart != 0.0) {//if the fragment is part of the outer object	
		pos = uViewPos + viewDir*(Ostart);//3d position of the current pixel on the outer object
	    vec3 refractedRay = normalize(refract(viewDir, Onormal, 1.0/1.33));
	    //Parameters for ray casting
		float lodLevelCone = tan( radians( 1.0 * 10.0 ) ) * 2.0 * 0.01;
		float init = 1.0 / 512.0;
		float Rfar = 0.99;
		float coneFactor = max( 1.0 / 2048.0, lodLevelCone );
    	refrColor = traceVoxelConeRayCast( pos, refractedRay, init, coneFactor, Rfar, normal );	
    	vec4 finalColor;
    	if (refrColor != vec4(0.0)) {
    		finalColor = refrColor;
    	} else {
    		finalColor = mix(vec4(refrColor.rgb, 0.3), texture(cubeTex, rotCube*(viewDir+refractedRay)), 0.8);
    	}

    	/**Reflection of cube map**/
    	vec3 reflectedRay = reflect(viewDir, Onormal);
    	vec4 envColor = texture(cubeTex, rotCube*(viewDir + reflectedRay));
    	vec4 reflColor = mix(finalColor, envColor, 0.1);

    	/**Specular lighting**/
    	vec3 lightVec = normalize( lightPos - pos );//point to light
        vec3 viewVec = -viewDir;// point to eye   
		vec3 reflVec = reflect(-lightVec, Onormal );//reflected light to point ray
		float cosAngle = max(0.0, dot(normalize(viewVec), reflVec));
		vec3 specular = vec3(0.0);
		if (max(0.0, dot (Onormal, lightVec)) > 0.0) {
			specular = pow(cosAngle, 100)*vec3(0.8);
		}

		gl_FragColor = reflColor + vec4(specular, specular.x);
	} 
}
