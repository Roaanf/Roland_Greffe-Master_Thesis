 ...
 *
 * @param MeshVertexTList data type list choosen by user (ex : (float4, uchar4) for position and color )
 * @param DataPageSize data page size in vertices (size is given by BVH_DATA_PAGE_SIZE)
 */
template< class MeshVertexTList, uint DataPageSize > // 
class BVHTrianglesManager 
{
...
public:
	...
	void generateBuffers( uint arrayFlag )
	{
		...
		//vertexBufferPool= new VertexBufferPoolType(make_uint3(trianglePagesTemp.size()*3), 2);	//Allocate mapped buffer
		...
		nodesBufferArray = new GvCore::Array3D< VolTreeBVHNode >( make_uint3( bvhNodesBufferBuildingList.size(), 1, 1 ), arrayFlag );	// Mapped memory
		for ( uint i = 0; i < bvhNodesBufferBuildingList.size(); ++i )
		{
			VolTreeBVHNode node = bvhNodesBufferBuildingList[ i ];
			//node.userNode.setGPULink();
			nodesBufferArray->get( i ) = node;
		}
		...
		vertexBufferPool = new GvCore::GPUPoolHost< GvCore::Array3D, MeshVertexTList >( make_uint3( vertexBufferSize, 1, 1 ), arrayFlag );
		...
		for ( uint i = 0; i < trianglePagesTemp.size(); ++i )
		{
			...
			vertexBufferPool->getChannel( Loki::Int2Type< 0 >() )->get( i ) = vertPosStore;
			...
			vertexBufferPool->getChannel( Loki::Int2Type< 1 >() )->get( i ) = vertColorStore;
#endif
		}
	...
	{
		nodesBufferArray->get( nodeAddress ).userNode.escapeIdx = escapeTo;

		VolTreeBVHNodeUser node = nodesBufferArray->get( nodeAddress ).userNode;
		if ( node.hasSubNodes() )
		{
  ...
  //	  }
...
#else
		glEnableClientState(GL_VERTEX_ARRAY);
		glVertexPointer(4, GL_FLOAT, 0, (float*) vertexBufferPool->getChannel(Loki::Int2Type<0>())->getPointer(0)  );

		glDrawArrays(GL_TRIANGLES, 0, vertexBufferPool->getChannel(Loki::Int2Type<0>())->getNumElements()/100 );
...
#else
		glBegin(GL_QUADS);
		for(uint i=0; i<this->nodesBufferArray->getSize().x; ++i){
			VolTreeBVHNode node=this->nodesBufferArray->get(i);
	...
	void recursiveRenderDebugGL(uint curNodeIdx)
	{
		...
		node=this->nodesBufferArray->get(curNodeIdx);
		...
		node=this->nodesBufferArray->get(curNodeIdx+1);
		if(node.userNode.isLinkActive()){
			if(node.userNode.isDataType()){
				glColor4f(1.0f, 0.0f, 0.0f, 1.0f);
				drawCube(node.userNode.bbox.pMin, node.userNode.bbox.pMax);
	...
	void displayTriangles(uint index)
	{
		...
		for(uint i=0; i<(BVH_DATA_PAGE_SIZE/3)*3; ++i){
			//std::cout<<i<<" ";
			float4 vpos=vertexBufferPool->getChannel(Loki::Int2Type<0>())->get(index*BVH_DATA_PAGE_SIZE + i);

			uchar4 vcol=vertexBufferPool->getChannel(Loki::Int2Type<1>())->get(index*BVH_DATA_PAGE_SIZE + i);
	...
	void renderFullGL()
	{
		...
		glEnd();
#else
		for(uint i=2; i<this->nodesBufferArray->getResolution().x; ++i){
			VolTreeBVHNode node;

			node=this->nodesBufferArray->get(i);
			if(node.userNode.isDataType()){
	 ...
	 * Get the node pool
	 */
	GvCore::Array3D< VolTreeBVHNode >* getNodeBufferArray()
	{
		return nodesBufferArray;
	}

	/**
	 * Get the data pool
	 */
	VertexBufferPoolType* getVertexBufferPool()
	{
