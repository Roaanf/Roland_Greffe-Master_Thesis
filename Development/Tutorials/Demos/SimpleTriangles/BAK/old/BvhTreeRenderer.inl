...
::doRender(const float4x4 &modelMatrix, const float4x4 &viewMatrix, const float4x4 &projMatrix, const int4& pViewport )
{
	...
	CUDA_SAFE_CALL(cudaMemcpyToSymbol(k_maxVolTreeDepth, (&this->_maxVolTreeDepth), sizeof(this->_maxVolTreeDepth), 0, cudaMemcpyHostToDevice));
//	CUDA_SAFE_CALL(cudaMemcpyToSymbol(k_userParam, (&this->_userParam), sizeof(this->_userParam), 0, cudaMemcpyHostToDevice));
	...
	CUT_CHECK_ERROR( "RenderKernelInitRayState" );
	...
	do
	{
		...
		CUDAPM_START_EVENT(vsrender_gigavoxels);
		...
		cudaDeviceSynchronize();
		CUT_CHECK_ERROR("RenderKernelContinueRays");

		CUDAPM_STOP_EVENT(vsrender_gigavoxels);
