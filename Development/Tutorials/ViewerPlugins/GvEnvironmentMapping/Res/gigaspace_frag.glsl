...
uniform float uReflectionCoeff;
uniform float uRefractionCoeff;
...
vec4 gv_traceVoxelConeRayCast( vec3 pRayStart, vec3 pRayDir, float t, float tMax, out vec3 pNormal );
...
void main()
{
	...
	vec4 materialColor = gv_traceVoxelConeRayCast( uViewPos, viewDir, initT, tfar, normal );

	if ( materialColor.w > 0.01 )
	{

	// Reflection
	/*vec3 normal = ;*/
	vec3 reflectionDir = reflect( -viewDir, normal );
	vec4 reflectedColor = texture( uSkyBoxTex, reflectionDir );
	//vec4 reflectedColor = vec4( 1.0, 0.0, 0.0, 1.0 );

	// Refraction
	vec3 refractionDir = refract( -viewDir, normal, uRefractionCoeff );
	vec4 refractedColor = texture( uSkyBoxTex, refractionDir );

	// Simulating reflection
	oColor = mix( materialColor, reflectedColor, uReflectionCoeff );
	// Simulating refraction
	//oColor = mix( refractedColor, reflectedColor, uRefractionCoeff );
	}
	else
	{
		oColor = materialColor;
	}
}
