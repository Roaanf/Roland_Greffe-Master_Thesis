...
uniform sampler3D uDataPoolChannel0;
uniform sampler3D uDataPoolChannel1;
...
vec4 gv_sampleBrickChannel0( uint brickIdxEnc, vec3 samplePos, vec3 nodePosition, float nodeSize );
vec4 gv_sampleBrickChannel1( uint brickIdxEnc, vec3 samplePos, vec3 nodePosition, float nodeSize );
...
////////////////////////////////////////////////////////////////////////////////
vec4 gv_sampleBrickChannel0( uint brickIdxEnc, vec3 samplePos, vec3 pNodePosition, float pNodeSize )
{
	...
	// Sample data
	return texture( uDataPoolChannel0, ( brickPos + samplePosBrick ) * uBrickPoolResInv ); // TO DO : optimize because of redundant computation of position : use a FMAD :)
}
...
// @return the sampled value at given position
////////////////////////////////////////////////////////////////////////////////
vec4 gv_sampleBrickChannel1( uint brickIdxEnc, vec3 samplePos, vec3 pNodePosition, float pNodeSize )
{
	// Unpack the brick address to obtain localization code
	uvec3 brickIdx;
	brickIdx.x = ( brickIdxEnc & 0x3FF00000U ) >> 20U;
	...
	// Sample data
	return texture( uDataPoolChannel1, ( brickPos + samplePosBrick ) * uBrickPoolResInv ); // TO DO : optimize because of redundant computation of position : use a FMAD :)
}
    ...
    while ( tTree < pTMax && /*stop criteria*/accColor.a < 0.99 && numLoop < uMaxNbLoops )
    {
		...
		if ( gv_nodeIsBrick( node ) )
        {
		    ...
		    while ( dt <= nodeLength && /*stop criteria*/accColor.a < 0.99 )
            {
				...
				if ( brickAddress != 0 )
				{
					color = gv_sampleBrickChannel0( brickAddress, samplePosTree, nodePosition, nodeSize );
				}
				else
				{
					if ( parentBrickAddress != 0 )
					{
						color = gv_sampleBrickChannel0( parentBrickAddress, samplePosTree, parentNodePosition, nodeSize * 2.0 );
					}
					else
					{	
						// DEBUG
				...
				if ( brickAddress != 0 )
				{
					normalVec = gv_sampleBrickChannel1( brickAddress, samplePosTree, nodePosition, nodeSize );
				}
				else
				{
					if ( parentBrickAddress != 0 )
					{
						normalVec = gv_sampleBrickChannel1( parentBrickAddress, samplePosTree, parentNodePosition, nodeSize * 2.0 );
					}
					else
					{
						// DEBUG
				...
				vec3 normal = normalize( vec3( normalVec.x, normalVec.y, normalVec.z ) );

	#endif // GV_USE_OPTIMIZED_NORMAL
