...
{	
	...
	{
		lcolor = vec4( 1.0, 0.0, 0.0, 1.0 ); //color of the object
	}
	vec3 pointToLight = vec3( lightPos - cameraPixelPos );
	vec3 N = normalize( normal );
	vec3 L = normalize( pointToLight );

	ambient = vec3( ambientLight.x * lcolor.x, ambientLight.y * lcolor.y, ambientLight.z * lcolor.z );

	diffuse = max( 0.0, dot( N, L ) ) * vec3( lcolor );
	...
	vec4 litColor = vec4( ambient + diffuse + specular, lcolor.w );
	gl_FragColor = litColor;
	
	// Shadow Casting
	vec4 rayTracedColor;
	vec3 worldRayDir = normalize( vec3( inverse( gvModelMatrix ) * vec4( worldLight, 1.0 ) ) - vec3( inverse( gvModelMatrix ) * ( worldPixelPos ) ) );
	...
	if ( hit )
	{
		...
		vec3 gvToLight = vec3( inverse( gvModelMatrix ) * vec4( worldLight, 1.0 ) ) - ( vec3( inverse( gvModelMatrix ) * ( worldPixelPos ) ) + initT * worldRayDir );
		// Only proceed if the GigaVoxels object is between this object and the lightsource
		if ( dot( worldRayDir, gvToLight ) > 0.0 )
		{
			rayTracedColor = traceVoxelConeRayCast( vec3( inverse( gvModelMatrix ) * ( worldPixelPos ) ), worldRayDir, initT, coneFactor, tfar );

			vec4 shadowColor = vec4( min( rayTracedColor.x * ( 1.0 - rayTracedColor.w ), 0.2 ),
									min( rayTracedColor.y * ( 1.0 - rayTracedColor.w ), 0.2 ),
									min( rayTracedColor.z * ( 1.0 - rayTracedColor.w ), 0.2 ),
									rayTracedColor.w );

			gl_FragColor = mix( shadowColor, litColor, 1.0 - shadowColor.w );
			//gl_FragColor = rayTracedColor;
		}
	}
}
