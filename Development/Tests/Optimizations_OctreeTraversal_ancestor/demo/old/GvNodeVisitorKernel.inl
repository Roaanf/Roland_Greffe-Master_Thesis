...
{
	 int x=(( *( int* ) &f.x ) & 0x7fffffff );
	 f.x = ( *( float* ) &x);
	 int y=(( *( int* ) &f.y ) & 0x7fffffff );
	 f.y = ( *( float* ) &y);
	 int z=(( *( int* ) &f.z ) & 0x7fffffff );
	 f.z = ( *( float* ) &z);
		...
		)
{
	...
	//uint optimalDepth = static_cast< uint >(( __logf( 1.f / brickRes ) - __logf( pConeAperture )) * 1.f / __logf( nodeRes )) - 3;
	//uint optimalDepth = static_cast< uint >(( __logf( brickRes ) + __logf( pConeAperture )) * 1.f / __logf(nodeRes) ) - 1;
	//uint nodeDepth = fminf( CACHE_DEPTH, optimalDepth );
...
#ifdef FLOAT_DESCENT
		//	nodeSizeTreeInv = 1.f / pNodeSizeTree;
		nodeSizeTreeInv = __powf( nodeRes, nodeDepth ) * 1.f / nodeRes;
#endif // FLOAT_DESCENT
	} else {
		// Alas, the point is not in the cache. Restart from the root.
		pNodeSizeTree = nodeRes;
